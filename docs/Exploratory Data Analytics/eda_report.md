...
All observations are pattern-based visual evidence — formal statistical testing and out-of-sample validation are reserved for Stages 3 and 4. The dataset is cross-sectional and observational, so no causal claims can be made from these correlations alone.
